...
export default async function DashboardPage() {
  ...
  const [totalProducts, brandCountsRaw, storageCountsRaw, products, settings, needs] =
    await Promise.all([
  ...
  const criticalPercent = settings?.criticalPercent ?? 50;

  const lowStock = products.filter((product) => {
    const current = product.currentStock ?? 0;
    if (useMinimumOrder) return current < (product.minimumOrder ?? 0);
  ...
  const criticalStock = products.filter((product) => {
    ...
    return (product.currentStock ?? 0) <= base * (criticalPercent / 100);
  });
    ...
    <section className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "Critical", value: criticalStock.length },
          { label: "Order needs", value: pendingNeeds.length }
        ].map((card) => (
